 ...
 * each of them. The alternative is one shared, network-wide summary table
 * holding a row per site, so the network can be read with a single query
 ...
 * (see schema.php, which also creates it), the same way core registers blogs
 * and sitemeta, so it is created once per network rather than once per site.
 *
