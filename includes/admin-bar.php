...
function wp_presence_admin_bar_node( $wp_admin_bar ) {
	...
	foreach ( $stack_ids as $stack_uid ) {
		...
		$avatar_url  = get_avatar_url(
			$user->ID,
			array(
				'size'    => wp_presence_get_avatar_fetch_size( 16 ),
				'default' => wp_presence_avatar_default(),
			)
		);
		$stack_html .= '<img src="' . esc_url( $avatar_url ) . '" width="16" height="16" style="z-index:' . (int) $z . '" alt="' . esc_attr( $user->display_name ) . '" title="' . esc_attr( $user->display_name ) . '" />';
		--$z;
	}
